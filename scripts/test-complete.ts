...
import { VisualDiffService } from '../src/lib/scanner/visual-diff';
  ...
  try {
    ...
    if (results.accessibility.length > 0) {
      const avgA11yScore = Math.round(
        results.accessibility.reduce((sum, r) => sum + r.score, 0) / results.accessibility.length
      );
      ...
      results.accessibility.forEach((r, i) => {
        ...
        if (r.violations.length > 0) {
          const criticalIssues = r.violations.filter((v: any) => v.impact === 'critical' || v.impact === 'serious');
          if (criticalIssues.length > 0) {
            console.log(`   Top Issues:`);
            criticalIssues.slice(0, 3).forEach((v: any) => {
    ...
    if (results.performance.length > 0) {
      const avgPerfScore = Math.round(
        results.performance.reduce((sum, r) => sum + r.score, 0) / results.performance.length
      );
      const avgLCP = Math.round(
        results.performance.reduce((sum, r) => sum + r.metrics.largestContentfulPaint, 0) / results.performance.length
      );
    ...
    if (results.aiCritique.length > 0) {
      const avgAIScore = Math.round(
        results.aiCritique.reduce((sum, r) => sum + r.score, 0) / results.aiCritique.length
      );
