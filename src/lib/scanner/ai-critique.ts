...
export interface AICritiqueOptions {
...
export class AICritiqueService {
  ...
  async critique(options: AICritiqueOptions): Promise<AICritiqueResult> {
  ...
  private buildPrompt(url: string, pageTitle?: string, context?: string): string {
...
Be specific and actionable. Identify both strengths and areas for improvement.`;
  ...
  private parseResponse(responseText: string, url: string): AICritiqueResult {
    try {
  ...
  private getFallbackResult(url: string): AICritiqueResult {
    return {
      url,
      score: 75,
...
export async function critiqueDesign(options: AICritiqueOptions): Promise<AICritiqueResult> {
