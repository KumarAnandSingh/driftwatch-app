...
export interface VisualDiffOptions {
  ...
  threshold?: number; // 0-1, higher = more sensitive
}
...
export class VisualDiffService {
   ...
   */
  async compare(options: VisualDiffOptions): Promise<VisualDiffResult> {
    const {
      baselineImagePath,
      currentImagePath,
      outputDiffPath,
    ...
    if (outputDiffPath || mismatchedPixels > 0) {
      ...
      diffImagePath = outputDiffPath || join(
        process.cwd(),
        'public',
        'artifacts',
        `diff_${timestamp}.png`
  ...
  async compareMultiple(
    pairs: Array<{ baseline: string; current: string; output?: string }>
   ...
   */
  async createBaseline(sourcePath: string, baselinePath: string): Promise<void> {
...
export async function compareImages(options: VisualDiffOptions): Promise<VisualDiffResult> {
